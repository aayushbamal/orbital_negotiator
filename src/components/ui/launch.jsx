...
function ShaderCanvas({
  ...
  useEffect(() => {
    ...
    const getDpr = () => {
      ...
      return Math.max(1, Math.min(4, pixelRatio ?? sys));
    };
    ...
    function onDown(e) { if (e.button === 0) mouseRef.current.l = 1; if (e.button === 2) mouseRef.current.r = 1; }
    function onUp(e)   { if (e.button === 0) mouseRef.current.l = 0; if (e.button === 2) mouseRef.current.r = 0; }
    ...
    function tick(now) {
      ...
      try {
        ...
        drawError(gl, err?.message ?? String(err));
      }
...
export default function Component({ pixelRatio }) {
  return (
    <div
      style={{
        position: "absolute",
